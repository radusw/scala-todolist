...
class TodoRepository(implicit xa: Transactor[Task]) extends TodoService[Task] {
  ...
  def create(partialTodo: (TodoId, Instant) => Todo): Task[Option[Todo]] = {
  ...
  def update(id: TodoId, patchTodo: Todo => Todo): Task[Option[Todo]] = {
    val q = for {
      ...
      updated = patchTodo(existing).copy(id = id, timestamp = Instant.now())
      r <- OptionT.liftF(
        sql"""
  ...
  def findAll(pagination: Pagination): Task[PaginationResult[Todo]] = {
    val q: ConnectionIO[PaginationResult[Todo]] = for {
      ...
      lst <- (
        selectFr ++
          fr"ORDER BY t.timestamp DESC, t.id ASC" ++
  ...
  def find(id: TodoId): Task[Option[Todo]] = {
    val q: ConnectionIO[Option[Todo]] = (selectFr ++ fr"WHERE t.id = $id").query[Todo].option
    q.transact(xa)
  }

  def delete(id: TodoId): Task[Option[Unit]] = {
    val q: ConnectionIO[Int] = sql"DELETE FROM todos t WHERE t.id = $id".update.run
